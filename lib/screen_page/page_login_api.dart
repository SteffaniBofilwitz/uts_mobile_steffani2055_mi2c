...
import 'package:uts_steffani2055/screen_page/page_bottom_navigation.dart';
import 'package:uts_steffani2055/screen_page/page_register_api.dart';
  ...
  Future<ModelLogin?> registerAccount() async {
    try {
      ...
      http.Response response = await http.post(
        Uri.parse('http://192.168.100.44/uts_mobile/login.php'),
        body: {
      ...
      if (data.value == 1) {
        setState(() {
          isLoading = false;
          session.saveSession(data.value ?? 0, data.idUser ?? "", data.username ?? "", data.nama ?? "",data.email ?? "", data.nobp ?? "", data.nohp ?? "", data.alamat ?? "" );
